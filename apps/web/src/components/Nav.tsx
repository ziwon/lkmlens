    ...
    <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur-sm dark:border-slate-800 dark:bg-slate-950/95">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3 sm:px-6">
        ...
        <nav aria-label="Primary" className="flex items-center gap-0.5 text-sm">
          <Link to="/signals" className="focus-ring rounded-md px-2.5 py-2 text-sm font-medium text-slate-600 sm:hidden dark:text-slate-300">Signals</Link>
          {links.map((link) => (
            <NavLink
              key={link.to}
              to={link.to}
              className={({ isActive }) => `focus-ring hidden rounded-md px-3 py-2 font-medium transition sm:block ${isActive ? "bg-slate-100 text-slate-950 dark:bg-slate-900 dark:text-white" : "text-slate-600 hover:bg-slate-100 hover:text-slate-900 dark:text-slate-300 dark:hover:bg-slate-900 dark:hover:text-white"}`}
            >
              {link.label}
            </NavLink>
          ))}
